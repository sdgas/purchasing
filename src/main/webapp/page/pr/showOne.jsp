...
<script type="text/javascript">
	$(function() {
		...
		if (${pr.status=='HEAD'}) {
			if (${person.position.positionId!=1} || test < 0) {
				$(".dis").css("display", "none");
			}
		}
		if (${pr.status=='LEADER'}) {
			...
			<table>
				<tr>
					...
					<td style="width: 500px"><c:if test="${pr.status == 'PR'}">
							...
							<span style="color: red">请购申请 -->招标采购部经理 -->分管领导 -->总经理
								--></span>
							<span>采购</span>
				...
				<tr class="dis">
					<td><input type="hidden" name="prId" value="${pr.prId}">
						<input type="radio" name="status" value="true"
						onchange="fillMsg('pass');">通过</td>
